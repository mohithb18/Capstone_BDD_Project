...
public class StepDefinitions {
	...
	// instance fields kept only for convenience/logging (but main storage in
	// TestContext)
	...
	public void the_user_fills_the_registration_form_with_username_email_and_password(String username, String email,
			String password) {
	...
	public void the_registration_attempt_should_be_recorded_success_or_graceful_failure_allowed() {
		...
		System.out.println("Recorded registration credentials -> username: " + TestContext.getRegUsername()
				+ ", email: " + TestContext.getRegEmail());
	}
	...
	public void the_user_logs_in_with_username_or_email_and_password(String usernameOrEmail, String password) {
		...
		if (usernameOrEmail == null || usernameOrEmail.trim().isEmpty() || usernameOrEmail.contains("auto")) {
			if (TestContext.getRegEmail() != null && !TestContext.getRegEmail().isEmpty())
				usernameToUse = TestContext.getRegEmail();
			else if (TestContext.getRegUsername() != null && !TestContext.getRegUsername().isEmpty())
				usernameToUse = TestContext.getRegUsername();
		}
	...
	public void the_login_attempt_should_be_recorded_logged_in_indicator_or_graceful_failure() {
		boolean haveCred = (TestContext.getRegEmail() != null && !TestContext.getRegEmail().isEmpty())
				|| (TestContext.getRegUsername() != null && !TestContext.getRegUsername().isEmpty());
		Assert.assertTrue(haveCred,
				"A credential (registered email or username) should have been available for the login attempt.");
	...
	public void the_following_program_links_should_be_visible(DataTable dataTable) {
		...
		Assert.assertTrue(home.areProgramLinksVisible(expected),
				"One or more expected program links were not visible.");
	}
	...
	public void at_least_one_resource_link_from_the_list_should_be_visible(DataTable dataTable) {
		...
		Assert.assertTrue(home.isAnyResourceLinkPresent(resources),
				"No expected resource links were found on the Home page.");
	}
